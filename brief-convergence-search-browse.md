...
`préprod-130.32`, après la première mise à l'échelle réelle du connecteur.
...
Sur les 253 392 objets de l'instance 130.32, à volume de résultats égal, **Meilisearch répond
...
`préprod-130.32`, l'utilisateur applicatif ne l'a pas (`USAGE` sur `*.*`), donc le repli par
...
- **une modification du socle n'est pas committée et ne peut pas l'être ici** :
  ...
  l'instance 130.32 de longue date ; la correction doit repartir en PR en amont, pas être committée dans le
